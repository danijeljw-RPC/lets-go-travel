...
Slice 5 implements this projection per hotel or flight component. Combined journeys retain distinct component status, canonical hash, version number, last reconciliation time and next flight departure; a supplier cancellation or failure moves the affected component to its explicit terminal state and the aggregate checkout to `RequiresSupport`.
...
The implemented `booking.booking_versions` table has a unique component/version-number key. EF rejects modified or deleted versions, and migration `20260808010000_Slice5BookingReconciliation` installs `booking.reject_booking_version_mutation()` so PostgreSQL rejects direct `UPDATE` or `DELETE` operations. The current component projection uses optimistic concurrency to make competing writes converge, while omitting a component/hash uniqueness constraint permits a legitimate later A-to-B-to-A state sequence. Raw webhook evidence remains in the private inbox and is not copied into canonical history.
...
Under [ADR-0008](../adr/accepted/ADR-0008-durable-flight-reconciliation-and-customer-notification.md), the dedicated flight-reconciliation worker checks every active future flight booking at least once per calendar day outside the final 24 hours before scheduled departure and at least once per hour during the final 24 hours before each affected flight segment. A daily check is brought forward to the 24-hour boundary when that boundary occurs first. The general worker applies a daily schedule to active hotels. Authenticated webhooks, booking completion and pending-operation recovery may enqueue immediate checks, and the Slice 5 migration backfills due work for active Slice 4 component bookings that already have a provider reference. One durable schedule row per component, atomic conditional claims and bounded leases preserve overdue work across restarts and prevent duplicate workers from processing the same due check normally. Supplier observation time rejects older states, and unique version numbers plus notification keys are the final convergence boundary.
...
The first successful retrieval records version 1 without email. Later schedule movement under 30 minutes is minor; movement of at least 30 minutes, an added or removed flight segment, flight number/airport, room/inclusion or cancellation-policy changes are material; cancellation, failed confirmation and hotel relocation are travel-blocking. Minor email respects 22:00–07:00 customer-local quiet hours, while material and travel-blocking items bypass it. The checked-in sender is deliberately unavailable and returns `notification_capability_unavailable`; processing stops in an inspectable failed state after eight attempts, and a live email route remains a production decision. The sender contract carries the durable outbox idempotency key.

`GET /api/v1/checkouts/{checkoutId}/history` requires the authenticated owner and returns ordered supplier-neutral metadata, flags and diffs. It excludes provider bindings/references, raw envelopes, canonical supplier evidence, notification destinations and internal errors.

## Failure and Retention

A failed attempt records time, supplier, category, retry eligibility, next retry, correlation ID, last known state and whether customer/support action is required. Inbox scheduling and supplier retrieval stop after eight attempts in a quarantined or failed state with an idempotent operational case; notification delivery likewise stops after eight attempts with an operational case. A quarantined inbox item or failed notification can be explicitly requeued by an operator; reconciliation work recovers through the same webhook/booking-triggered enqueue path used for normal scheduling. Failure to reconcile never implies that the booking is unchanged.

Canonical history and raw supplier evidence have different retention periods under [Data Retention and Legal Hold](../security/data-retention-and-legal-hold.md). Canonical booking/financial evidence uses the seven-year baseline. Successful allowlisted raw supplier payloads expire after 90 days; ambiguous-operation, mapping-failure, incident or dispute payloads expire 12 months after resolution unless a legal hold or stricter requirement applies. Closed [OI-0011](../issues/closed/OI-0011-supplier-payload-retention.md) retains the LiteAPI contractual and Australian legal/privacy production gates. Slice 7 (see [Retention and Legal Hold](retention-and-legal-hold.md)) encodes all three periods in the retention policy catalog but implements no live sweep for them, since production supplier integration remains disabled and no live raw-payload or canonical-evidence data exists yet to sweep; it does implement the 90-day webhook-payload-body sweep, which redacts `WebhookInboxItem.RawBody` while retaining event identity, hash, correlation and processing outcome for the documented 2-year window.
