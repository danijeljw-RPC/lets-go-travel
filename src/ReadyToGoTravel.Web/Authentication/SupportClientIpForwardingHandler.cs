...
// The anonymous support-ticket-create API endpoint rate-limits per client IP. Blazor Server means
// every call this host makes to the API originates from this host's own connection, not the
// browser's, so without this handler every visitor sharing a Web deployment collapses into one
// rate-limit bucket. Forwards the browser's address as seen by this host (itself only reliable if
// this host is not sitting behind its own unconfigured reverse proxy - a separate, existing
// concern this handler does not attempt to solve) alongside a shared secret the API only trusts
// from this configured value, so a caller other than this Web host cannot spoof a fresh bucket.
