...
The implemented `booking.booking_versions` table has unique component/version-number and component/hash keys. EF rejects modified or deleted versions, and migration `20260808010000_Slice5BookingReconciliation` installs `booking.reject_booking_version_mutation()` so PostgreSQL rejects direct `UPDATE` or `DELETE` operations. Raw webhook evidence remains in the private inbox and is not copied into canonical history.
...
Under [ADR-0008](../adr/accepted/ADR-0008-durable-flight-reconciliation-and-customer-notification.md), the dedicated flight-reconciliation worker checks every active future flight booking at least once per calendar day outside the final 24 hours before scheduled departure and at least once per hour during the final 24 hours before each affected flight segment. The general worker applies a daily schedule to active hotels. Authenticated webhooks, booking completion and pending-operation recovery may enqueue immediate checks. One durable schedule row per component, atomic conditional claims and bounded leases preserve overdue work across restarts and prevent duplicate workers from processing the same due check normally. Unique versions and notification keys are the final convergence boundary.
...
The first successful retrieval records version 1 without email. Later schedule movement under 30 minutes is minor; movement of at least 30 minutes, flight number/airport, room/inclusion or cancellation-policy changes are material; cancellation, failed confirmation and hotel relocation are travel-blocking. Minor email respects 22:00–07:00 customer-local quiet hours, while material and travel-blocking items bypass it. The checked-in sender is deliberately unavailable and retries with `notification_capability_unavailable`; a live email route remains a production decision. The sender contract carries the durable outbox idempotency key.
