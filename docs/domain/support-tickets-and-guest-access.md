...
Slice 6 stored the fields retention enforcement needs — ticket closure timestamps and attachment creation timestamps. Slice 7 implements that enforcement: `SupportRetentionSweepProcessor` deletes general tickets 2 years after closure, booking-related tickets 7 years after closure, and purges attachments (storage object plus row plus usage counters, atomically) 90 days after their ticket's closure — all recorded in [Data Retention and Legal Hold](../security/data-retention-and-legal-hold.md) and detailed in [Retention and Legal Hold](retention-and-legal-hold.md). A legal hold scoped to a ticket suspends every one of these actions for that ticket until released. Deleting a ticket's message history required relaxing the Slice 6 append-only trigger on `support_ticket_messages` to reject `UPDATE` only (it previously also rejected `DELETE`, which blocked the sweep entirely in production — see the [Slice 7 outcome report](../delivery/2026-08-10-slice-7-retention-privacy-production-readiness-outcome.md)); message content immutability during a ticket's active life is unaffected.
